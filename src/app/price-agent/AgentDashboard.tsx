  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <section className="mx-auto max-w-6xl p-6 md:p-10">
      ...
      {result && <section className="mt-7 grid gap-6 lg:grid-cols-2">
        ...
        {result.aggregates.length > 0 && <article className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2"><h2 className="text-xl font-bold">Calculated database results</h2><p className="mt-1 text-sm text-slate-500">Each value is a price per quantity_standardized unit.</p><table className="mt-4 w-full min-w-[820px] text-left text-sm"><thead className="border-b text-slate-500"><tr><th className="p-2">Product</th><th className="p-2">Country</th><th className="p-2">Scale</th><th className="p-2">Average</th><th className="p-2">Median</th><th className="p-2">Range</th><th className="p-2">Records</th></tr></thead><tbody>{result.aggregates.map((item) => <tr key={`${item.product}-${item.country}-${item.scale}`} className="border-b last:border-0"><td className="p-2 font-medium">{item.product}</td><td className="p-2">{item.country}</td><td className="p-2">{item.scale}</td><td className="p-2">{item.averageUnitPrice.toLocaleString()}</td><td className="p-2">{item.medianUnitPrice.toLocaleString()}</td><td className="p-2">{item.minimumUnitPrice.toLocaleString()} – {item.maximumUnitPrice.toLocaleString()}</td><td className="p-2">{item.recordCount.toLocaleString()}</td></tr>)}</tbody></table></article>}
      </section>}
    </section>
  </main>;
}
